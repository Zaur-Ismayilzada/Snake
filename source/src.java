...
public class aufgabe_9 extends PApplet {
...
public void setup(){
  main = new SoundFile(this, "theme.mp3");
  eat = new SoundFile(this, "apple.mp3");
  game_over = new SoundFile(this, "game_over.mp3");
  ...
  apple_x = PApplet.parseInt(random(0, 25)) * 32;
  apple_y = PApplet.parseInt(random(0, 19)) * 32;
...
public void updateBoard(){
  for(Area2D temp : snake){
    if(temp != null && isInBoard(temp)){
      board[PApplet.parseInt(temp.getX() / 32)][PApplet.parseInt(temp.getY() / 32)] = 1;
    }
    else
      break;
  }
...
public int toGrid(float num){
  return PApplet.parseInt(num / 32);
}
...
class Area2D{
  ...
  public void settings() {  size(800, 608); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "aufgabe_9" };
    if (passedArgs != null) {
